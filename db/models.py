...
class Column(models.Model):
    class ColumnTypes(Enum):
        ...
        def validate(cls, column_type: str, value: Any) -> None:
            ...
            if (
                ...
                or (
                    column_type == Column.ColumnTypes.TIME.value
                    and not re.match(time_regex, value)
                )
                or (
                    column_type == Column.ColumnTypes.TIME_INTERVAL.value
                    and not re.match(time_regex, value.split(" - ")[0])
                )
            ):
    ...
    COLUMN_DEFAULTS = {
        ...
        ColumnTypes.TIME.value: datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        ColumnTypes.TIME_INTERVAL.value: "00:00:00 - 00:00:01",
    }
